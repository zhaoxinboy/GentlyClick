...
- (QDRclassiDataModel *)classDataModelForRow:(NSInteger)row{
    NSLog(@"%@", self.classDataArr[row]);
    return (QDRclassiDataModel *)self.classDataArr[row];
}
...
- (NSURL *)classiDataaddressURLForRow:(NSInteger)row{
    NSLog(@"%ld", (long)row);
    QDRclassiDataModel *model = [self classDataModelForRow:row];
    NSString *path = model.appurl;
    return [NSURL URLWithString:path];
}
